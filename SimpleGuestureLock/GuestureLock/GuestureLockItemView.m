...
@interface GuestureLockItemView()
...
@implementation GuestureLockItemView
...
-(void)propertySetting:(CGContextRef)ctx{
    ...
    if(_selected){
        ...
        color = GuestureLockCircleLineSelectedColor;
    }else{
        color = GuestureLockCircleLineNormalColor;
    }
    [color set];
}
...
-(void)circleSelected:(CGContextRef)ctx rect:(CGRect)rect{
    ...
    [GuestureLockCircleLineSelectedCircleColor set];
...
-(CGRect)calRect{
    ...
    if(CGRectEqualToRect(_calRect, CGRectZero)){
        ...
        CGFloat lineW = GuestureLockArcLineW;
...
-(CGRect)selectedRect{
    ...
    if(CGRectEqualToRect(_selectedRect, CGRectZero)){
        ...
        CGFloat selectRectWH = rect.size.width * GuestureLockArcWHR;
        
        CGFloat selectRectXY = rect.size.width * (1 - GuestureLockArcWHR) *.5f;
